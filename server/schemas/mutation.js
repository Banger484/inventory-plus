...
const mutation = {
  ...
  updateItemSale: async (
    parent,
    { item, saleDate, buyer, saleNumber, salesPrice }
  ) => {
    return await Item.findOneAndUpdate(
      { _id: item },
  ...
  register: async (
    parent,
    { name, email, password, location, enterpriseName }
  ) => {
    try {
  ...
  addToOrderGuide: async (parent, { enterpriseId, productId }) => {
    ...
    await ent.save();
    const result = await Enterprise.findById(enterpriseId).populate(
      "orderGuide"
    );
    return result;
  },
